...
**Agent-chaining fix→review (Fase 8D, opt-in):** il reusable espone l'input `chain_review` (default **off**). Quando un caller PR-creator lo imposta a `true`, dopo l'apertura della PR applica la label `agent:review` alla PR → la code review parte da sola. È **disattivato di default** per non moltiplicare run/consumi. In alternativa resta sempre possibile applicare `agent:review`/`agent:security` a mano.

> ⚠️ **Stato: meccanismo implementato e cablato, ma attivazione bloccata dal token.** Perché il chaining inneschi davvero la review servono **due** proprietà sul token che applica la label: (1) deve essere un token "utente" (PAT/App), perché gli eventi del `GITHUB_TOKEN` di default **non innescano** altri workflow (anti-ricorsione di GitHub); (2) deve avere **scope repo `issues:write`**. Validazione e2e sul sandbox: con l'attuale `MASTER_BOARD_TOKEN` (scope *project*) l'API `addLabelsToLabelable` risponde *"Resource not accessible"* → il reusable degrada con un warning chiaro e non rompe il job, ma la review non parte. **Sbloccare il chaining è parte della decisione G2** (token con scope repo o GitHub App token) → vedi *⚠️ Da valutare con il team*. Finché non è risolto, `chain_review` resta off ovunque.
...
- **`MASTER_BOARD_TOKEN` è un PAT statico.** Anthropic e i post-mortem di supply-chain (Clinejection 2026) avvertono che un token statico è teoricamente recuperabile nel tempo via prompt-injection e **non ruota**. Mitigazioni da valutare: (a) renderlo **fine-grained con scadenza breve** e scope minimo, oppure (b) sostituirlo con un **GitHub App token** generato al volo per-run. Nota: oggi il PAT è già scopato per-step (non raggiunge lo step Claude), quindi il residuo è basso ma non nullo. **Decisione collegata**: il token attuale ha scope *project* e **non** `issues:write` → blocca il chaining 8D (vedi sopra). Se il team sceglie un GitHub App token o un fine-grained con scope repo, **si sblocca anche il chaining fix→review** oltre a ridurre il rischio del PAT statico.
